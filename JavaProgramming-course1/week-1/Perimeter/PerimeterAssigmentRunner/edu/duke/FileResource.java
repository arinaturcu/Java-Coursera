...
public class FileResource {
     ...
     * @throws exception if no file is selected by the user
     */
    public FileResource (final boolean writable) {
        if (writable) {
            initWrite();
        }
        else {
     ...
     * @throws exception if the file cannot be accessed
     */
    public FileResource (final File file, final boolean writable) {
        if (writable) {
            initWrite(file);
        }
        else {
     ...
     * @throws exception if the filename cannot be accessed
     */
    public FileResource (final String filename, final boolean writable) {
        if (writable) {
            initWrite(filename);
        }
        else {
     ...
     * @param list the strings to saved to the file
     */
    public void write (final String[] list) {
        // BUGBUG: yuck :(
     ...
     * @param list the strings to saved to the file
     */
    public void write (final ArrayList<String> list) {
        if (mySaveFile != null) {
    ...
    private void initRead () {
        final File f = FileSelector.selectFile();
        if (f == null) {
    ...
    private void initRead (final File f) {
        try {
    ...
    private void initRead (final String fname) {
        try {
            myPath = fname;
    ...
    private String initFromStream (final InputStream stream) {
    ...
    private void initWrite () {
        final File f = FileSelector.saveFile();
        if (f == null) {
    ...
    private void initWrite (final File f) {
        try {
            mySaveFile = f;
    ...
    private void initWrite (String fname) {
        try {
            final URL loc = getClass().getClassLoader().getResource(fname);
            if (loc != null) {
                fname = loc.toString();
            }
